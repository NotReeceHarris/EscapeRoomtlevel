...
def Room2Start(mainCharacter, sideCharacter, antagonistCharacter, scene, roomName1, roomName2, roomName3, roomName4, roomName5, specialRoom, inventorySpace, specialItem, commandLine, difficulty, escapeDoorLocation):
    ...
    if UserInput.lower() == "help":
      with open(jsonInventory, 'r') as e:
        ...
        if "trapdoor" in inventoryitem1["unlocks"]:
          print("keypad      -> Inspect KeyPad\n\n    --------------------------\n")
        else:
          print("trapdoor    -> Inspect TrapDoor\n\n    --------------------------\n")
        if "Key Half 2 (Room 3)" in inventoryitem1["items"] and "Key Half 1 (Room 3)" in inventoryitem1["items"]:
          print("craftkey   -> Combined key halfs\n\n    --------------------------\n")
      ...
      if inventoryitem1["locations"]["keylocation"] == "item1room2":
        if "item1room2" in inventoryitem1["unlocks"]:
          ...
          print(random.choice(dialog["pickupkey"]).format("Key Half 2 (Room 3)"))
      ...
      if "trapdoor" in inventoryitem5["unlocks"]:
        ...
        if "Black Light" in inventoryitem5["items"]:
            print(random.choice(["You lift the drop door theres a keypad, you use the blacklight on the keys, and see finger print on the keys {} {} {} {}", "Its a trapdoor! * You pull a 10000 iq play * il use the blacklight to see finger prints, you shine the light on the keypad and see the finger prints on the keys {} {} {} {}"]).format(inventoryitem5["otherdata"]["room2"][0], inventoryitem5["otherdata"]["room2"][1], inventoryitem5["otherdata"]["room2"][2], inventoryitem5["otherdata"]["room2"][3]))
      ...
      if "trapdoor" in inventoryitem5["unlocks"]:
        if "keypad" in inventoryitem5["unlocks"]:
          ...
          while True:
            userinput = input(f"Keypad: 4 digit       |  Known Numbers: {inventoryitem5['otherdata']['room2'][0]} {inventoryitem5['otherdata']['room2'][1]} {inventoryitem5['otherdata']['room2'][2]} {inventoryitem5['otherdata']['room2'][3]} \n>#> ")
            if not userinput.isnumeric():
              ...
              print(random.choice(["beep, beep, beep, Wait that worked? theres a key inside, wait no its broken maybe the other half is around here."]))
